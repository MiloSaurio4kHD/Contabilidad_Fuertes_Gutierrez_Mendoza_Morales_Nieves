...
{
    public partial class frmAgregarAsientoLibroDiario : Form
    {
        ...
        public AsientoContable Asiento { get; private set; }
        ...
        /// </summary>
        public frmAgregarAsientoLibroDiario() : this(ModoFormulario.Crear, 1)
        {
        }

        public frmAgregarAsientoLibroDiario(ModoFormulario modo, int numero, AsientoContable asientoExistente = null)
        {
            InitializeComponent();
            ...
            {
                Text = "Agregar asiento";
                btnAgregarAsiento.Text = "Guardar asiento";
                dtpFechaAsiento.Value = DateTime.Today;
            }
            else
            {
                Text = "Editar asiento";
                btnAgregarAsiento.Text = "Guardar cambios";

                if (asientoExistente == null)
                    throw new ArgumentNullException(nameof(asientoExistente), "Debe proporcionar el asiento a editar.");

                dtpFechaAsiento.Value = asientoExistente.Fecha;
                txtGlosa.Text = asientoExistente.Glosa;
                foreach (var detalle in asientoExistente.Detalles)
                {
        ...
        private void ConfigurarIconos()
        {
            ...
            UI.IconHelper.AplicarIconoBoton(btnAgregarAsiento, "Save.ico");
        }

        private void ConfigurarGridDetalle()
        {
            dgvAsientoIndividual.AutoGenerateColumns = false;
            dgvAsientoIndividual.Columns.Clear();
            dgvAsientoIndividual.ReadOnly = false;
            dgvAsientoIndividual.AllowUserToAddRows = false;
            dgvAsientoIndividual.AllowUserToDeleteRows = false;
            dgvAsientoIndividual.RowHeadersVisible = false;
            dgvAsientoIndividual.SelectionMode = DataGridViewSelectionMode.FullRowSelect;
            dgvAsientoIndividual.AutoSizeColumnsMode = DataGridViewAutoSizeColumnsMode.Fill;

            dgvAsientoIndividual.Columns.Add(new DataGridViewTextBoxColumn
            {
                Name = "colCuenta",
                HeaderText = "Cuenta",
                ReadOnly = true,
            ...
            dgvAsientoIndividual.Columns.Add(new DataGridViewButtonColumn
            {
                Name = "colEditar",
        ...
        {
            dgvAsientoIndividual.Rows.Clear();
            foreach (var detalle in _detalles)
            {
                int idx = dgvAsientoIndividual.Rows.Add(
                ...
                dgvAsientoIndividual.Rows[idx].Tag = detalle;
            }
        ...
        private void frmAgregarAsientoLibroDiario_Load(object sender, EventArgs e)
        {
        }
        ...
        private void rbDebe_CheckedChanged(object sender, EventArgs e)
        {
        }

        private void rbHaber_CheckedChanged(object sender, EventArgs e)
        {
        }
        ...
        private void dtpFechaAsiento_ValueChanged(object sender, EventArgs e)
        {
        }
        ...
        private void dgvAsientoIndividual_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex < 0) return;
            string nombreColumna = dgvAsientoIndividual.Columns[e.ColumnIndex].Name;
            if (nombreColumna != "colQuitar" && nombreColumna != "colEditar") return;

            var detalle = dgvAsientoIndividual.Rows[e.RowIndex].Tag as DetalleAsiento;
        ...
        private void btnAgregarAsiento_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(this, "El asiento debe tener al menos dos líneas (una en el Debe y otra en el Haber).",
                    "Asiento incompleto", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            if (string.IsNullOrEmpty(glosa))
            {
                MessageBox.Show(this, "Debe ingresar la glosa del asiento.",
                    "Asiento incompleto", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            {
                MessageBox.Show(this,
                    string.Format("El asiento no está balanceado. Debe: {0:N2}  Haber: {1:N2}", totalDebe, totalHaber),
                    "Asiento descuadrado", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }
            ...
            {
                Numero = _numero,
                Fecha = dtpFechaAsiento.Value.Date,
                Glosa = glosa,
